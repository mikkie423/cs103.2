...
//Welcome Function by Delear for the donor user
//Main screen donor sees when logged in
//users have 5 options to choose from
void welcome()
{
	...
	{
	case 1: eligibilityTest();  //eligibilitytest function
		break;
	case 2: bookAppt(); //booking appts function
		break;
	case 3: viewAppt();  //viewing appts function to view, edit or cancel
		break;
	case 4:
		editUser(loggedinUser->username);  //user editing their account
		break;
	case 5:
		loggedinUser = {}; //reset the structure as user logs out
		rego();
...
//eligibility function by Delear
//all donors must do this by mandatory if they want to book an appt
void eligibilityTest()
{
	...
	if (loggedinUser->booking.d == 1) //d = 1 means they already booked an appoint
	{
		...
		cin >> op;  //char op to re-direct user if they already booked an appointment
		if (op == 'y')
		{
			viewAppt();  //re-directing user to view the appointment they already booked or back main menu
		}
		else if (op == 'n')
		{
			welcome();
	...
	for (int i = 0; i < 7; i++)  //for loop to loop through the questions within the array
	{
		...
		cin >> a;   //char a for eligibility questions
		...
		if (a == 'y')
		{
			loggedinUser->booking.e = 0;  //failing the eligibility will reset booking.e to 0, they will have to do it again
	...
	cin >> b;  //char b helping user to decide whether they want to book or go back to main menu
...
//booking appt function by Delear & Michaela
//users can book an appt if they have passed the eligibility test
void bookAppt()  //booking appointments function
{
	...
	if (loggedinUser->booking.e == 0 && loggedinUser->booking.d == 0)  //d = 0 & e = 0 redirects them to the eligibilty test if they haven't done it already
	{
		...
		cin >> c;  //char c will re-direct user to either the eligibility test or back to main menu
		if (c == 'y')
		{
			eligibilityTest();
		}
	...
	if (loggedinUser->booking.d == 1)  //d = 1 means they already booked an appointment
	{
		cout << "\nYou already booked an appointment" << endl;
		cout << "\nif you would like to view or cancel an appointment press y, or press n to go back to the main page: ";
		cin >> op;  //char op to let users know they already have an appointment
		if (op == 'y')
		{
			viewAppt();
		}
	...
	if (loggedinUser->booking.location != "auckland" && loggedinUser->booking.location != "wellington" && loggedinUser->booking.location != "christchurch")
	{
		...
		cin >> loggedinUser->booking.location;  //loggedinUser copies to txt file
	...
	if (loggedinUser->booking.date <= 20220620 || loggedinUser->booking.date >= 20240620) //int & condition to help user pick proper date
	{
...
//viewing appt function by Delear & Michaela
//users can view appts or edit, 
//using local variables, structs and vectors
void viewAppt()  //viewing, ediiting or cancelling appt function
{
	char a, b, op;  //local variables

	if (loggedinUser->booking.e == 0)  //cannot access this page without having done an eligibility test and booked an appt
	{
		cout << "\nYou must be eligible first before you can donate, \nType y to go to the eligibility test \nType n to go back to main page " << endl;
		cin >> b;
		if (b == 'y')
	...
	if (loggedinUser->booking.d == 0) //cannot access this page without having booked an appt
	{
	...
	cout << "\n\n" << loggedinUser->username;  //viewing details
	cout << "\n" << loggedinUser->bloodGroup;
	cout << "\n" << loggedinUser->booking.location;
	...
	do  //do-while loop incase they fail to answer any of the following options
	{
		...
		cin >> a;  //char a gicing options to book, edit or cancel
		if (a == 'y')
		{
			cout << "\nThank you for choosing to book an appointment" << endl;
			welcome();
		...
		{
			cout << "\nYou chose not to book an appointment" << endl;
			deletefile(loggedinUser->username);  //redirecting to cancelling appt
		}
		else if (a == 'e')
		{
			cout << "\nYou chose edit an appointment" << endl;
			editAppt();  //redirecting to editing appt
		}
	} while (a != 'y' && a != 'n' && a != 'e');
...
//editing appt by Delear and Michaela
//users can edit their appt
//file sharing to update txt file
void editAppt()  //editing appt function
{
	...
	ofstream updateFile;  //new info is copied to a file and replaces the old info
...
//Cancelling appts by Delear and Michaela 
//users can cancel an appt
//overwrites txt file and deletes the appt
void deletefile(string user)  //cancelling appt function
{
	char choice = 'f';  //local variables
	getAllLogins();
	...
	for (auto it = users.begin(); it != users.end(); it++)
	{
		if ((*it).booking.d == 1)  
		{
			if ((*it).username == user) //iterator to pointer
			{
				system("CLS");
				...
				{
					(*it).booking.date = NULL;  //resets users appt to 0
					(*it).booking.location = "";
	...
	ofstream deleteFile;  //delete info in the txt file
